...
import {
  ...
  const PortalUsers = (): JSX.Element => {
    ...
    const filteredUsers = activeFilter === true ? portalUsers.filter(user => user.active) : portalUsers.filter(user => !user.active);
            ...
            <div>
              <div className="sm:ml-6 sm:block">
                <div className="flex space-x-4">
                  ...
                  <button onClick={() => handleNewUserClick()} className="rounded-md bg-[#0284c7] outline outline-1 outline-gray-400 px-4 py-2 text-md font-medium text-gray-50">
                    New User
                  </button>
                </div>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {filteredUsers.map((user) => (
                      <tr key={user.email} onClick={() => handleActiveUserClick(user.id)}>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 text-center">
                          <span className={`inline-flex items-center rounded-full px-3 py-0.5 text-xs font-normal ${user.role === "Store" ? "text-[#075985] bg-[#e0f2fe] " : "text-[#065f46] bg-[#d1fae5]"}`}>
                            {user.role}
                          </span>
                        </td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {user.stores.length > 1 ? <span className="font-bold">{user.stores.length} stores</span> : user.stores.map((store) => (<span>{store}</span>))}
                        </td>
                      </tr>
                    ))}
                  </tbody>
